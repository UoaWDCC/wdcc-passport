...
/** Zoomed card size relative to pokebox's single-card size: smaller so it reads as further away on a laptop. */
export const ZOOMED_SCALE = 0.72;
...
export function zoomedTransform(dims: SceneDims): Transform {
  ...
  return {
    ...
    ry: 0,
    rz: 0,
    scale: dims.screenH * singleCardSize(dims) * ZOOMED_SCALE,
  };
}
