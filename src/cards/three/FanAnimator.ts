  ...
  tick(entries: Map<number, FanCardEntry>, now: number, dt: number, ctx: FanTickContext): void {
    ...
    for (const entry of entries.values()) {
      ...
      if (entry.intro) {
        ...
        // Cards in the hand do not follow the pointer; only the zoomed card tilts.
        g.rotation.set(0, 0, from.rotZ + (rest.rotZ - from.rotZ) * e);
      ...
      // No pointer tilt on hand cards (pokebox used 0.5 / 0.3 of it); keep the little scroll swing.
      g.rotation.set(0, ctx.scrollTilt * 0.18, rest.rotZ + (hover.rotZ - rest.rotZ) * l);
